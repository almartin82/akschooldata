...
**Why it matters**: Code doesn’t run during CI, broken code goes
undetected **Current status**: Documented, ready for action
...
**Pattern**: Package installation commands **Why eval=FALSE**: Users run
these manually, not during vignette build **Action**: Keep as-is, maybe
add explanatory comment

**Packages**: - arschooldata (line 27): `install` chunk - rischooldata
(line 23): `install` chunk
...
1.  **Priority**: Should I fix eval=FALSE issues or focus on Rd widths
    first?

2.  **Approach**:

    - 1.  Fix all eval=FALSE systematically (4-6 hours)
    - 2.  Fix only high-priority packages (ar, ca, il, ny, tx) (2-3
          hours)
    - 3.  Just add documentation comments (1 hour)
    - 4.  Defer and focus on Rd widths
...
**Pending**: Your decision on approach 📋 **Ready**: Action plan and
scripts (once you decide)
